...
(2)TIMER_FUNC_TEST：
        用于测试第一段计时是自定义，后续计时是指定时长的计时方案。此功能用于nscp接收器的采样时钟。
        具体实验内容是设置单位计时间隔为0.5us，指定计数为500，初始计数寄存器值为300，即第一段计数为200。共产生五次中断，中断中改变PA6的电平，通过示波器查验。
        预计现象是1段100us(0.5 x 200)左右的计时区间+4段250us(0.5 x 500)左右的计时区间。
(3)TRIGGER_FUNC_TEST：
        用于测试定时器随外部的一个上升沿触发开始计时。此功能用于nscp接收器何时启动采样时钟。
        具体实验内容为，VCC通过一个点触式按钮连接到PA7。当按下按钮，PA7上拉至高电平，上升沿触发TIM3定时器启动定时，定时结果输出到PA6引脚。
        预计现象是示波器上，PA7上升沿与PA6的第一个上升沿间隔100us，后续PA6两个翻转沿边的间隔为250us共四段。
